...
mortality <- read.csv("HIV_deaths.csv", fileEncoding = "UTF-8")
overview <- read.csv("HIV_diagnoses.csv", fileEncoding = "UTF-8")
m_1 <- read.csv("m_1.csv", fileEncoding = "UTF-8")
m_2 <- read.csv("m_2.csv", fileEncoding = "UTF-8")
m_3 <- read.csv("m_3.csv", fileEncoding = "UTF-8")
f_1 <- read.csv("f_1.csv", fileEncoding = "UTF-8")
...
overview_pie3 <- function(data) {
  filtered_data <- 
    ...
    mutate(Percent = Cases / sum(Cases) * 100)
  color_palette <- brewer.pal(n = 7, name =  "Blues")
  plot_ly(filtered_data, values = ~Cases, labels = ~ Race, type = "pie",
          textinfo = "percent",hole=0.6, marker = list(colors = color_palette)) %>%
    layout(title = "HIV Diagnoses by Race",
           showlegend = FALSE,
           plot_bgcolor = "#f6f6f6",
           paper_bgcolor = "#f6f6f6")
}
